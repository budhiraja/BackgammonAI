...
PS. Do not run on Delicate Machines. 
"""
	...
	if bear_off:
		for i in range(18,24):
				...
				if i+d>24 and bear_child.board[i]>0:
					...
					bear_child.move=root.move+str(i+1)+" "+str(0)+'\n'
		...
		if bear_off:
			for i in range(18,24):
				...
				if i+d==24 and new_bear_child.board[i]>0 :
					...
					new_bear_child.move=root.move+str(i+1)+" "+str(0)+'\n'
...
def init(board,bar,dice):	# bar[0] is mine and bar[1] is opponent's
	...
	if rootResults!=[]:
		for i in rootResults:
			...
			for j in dice_combo:
				temp=rev(i)
				depth2_temp=makeMoves(temp,j)
				depth1_result+=depth2_temp
				for x in depth2_temp:
					counter=0
					depth2_result=[]
					sum_of_dice=0.0
					for y in roller_dice:
						temp2=rev(x)
						temp3=makeMoves(temp2,y.value)
						depth2_result+=temp3
						if temp3 != []:
							#print "printing Z"
							depth3_max_child=temp3[0]
							for z in temp3:
								#print z.board
								if evaluation(z) > evaluation(depth3_max_child):
									depth3_max_child=z
							y.max_value=evaluation(depth3_max_child)
							sum_of_dice+=y.max_value
							counter+=1
					if counter!=0:
						x.evaluation=sum_of_dice/counter
					else:
						x.evaluation=-5        # Least Evaluation Due to the fact that "we" cant move
			for k in depth1_result:
				temp_evaluation+=k.evaluation
